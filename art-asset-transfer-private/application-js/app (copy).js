...
async function getAllAssets(org) {
    ...
    var ccpOrg = undefined;
    var walletOrg = undefined;
    var OrgUserId = undefined;
    if (org==1){
        ccpOrg = ccpOrg1
        walletOrg = walletOrg1
        OrgUserId = Org1UserId
    } else if (org=2){
        ccpOrg = ccpOrg2
        walletOrg = walletOrg2
        OrgUserId = Org2UserId
    } else {
        return '{"type":"Error!Org does not exist!"}';
    }
    ...
    try {
        ...
        await gatewayOrg.connect(ccpOrg,
            { wallet: walletOrg, identity: OrgUserId, discovery: { enabled: true, asLocalhost: true } });
        const networkOrg = await gatewayOrg.getNetwork(myChannel);
        const contractOrg = networkOrg.getContract(myChaincodeName);
        
        console.log('\n--> Evaluate Transaction: GetAssetByRange asset0-asset9');
        // GetAssetByRange returns assets on the ledger with ID in the range of startKey (inclusive) and endKey (exclusive)
        result = await contractOrg.evaluateTransaction('GetAssetByRange', 'asset0', 'asset9');
        console.log('  result: ' + prettyJSONString(result.toString()));
...
async function getAssetPrivateData(org, id){
    ...
    try {
        ...
        result = await contractOrg.evaluateTransaction('ReadAssetPrivateDetails', orgPrivateCollectionName, id);
        console.log('  result: ' + prettyJSONString(result.toString()));
...
async function getAssetTransferAgreement(org, id){
    ...
    try {
        ...
        result = await contractOrg.evaluateTransaction('ReadTransferAgreement', id);
        console.log('  result: ' + prettyJSONString(result.toString()));
        if (result == null || result == ''){
            console.log('[]');
            gatewayOrg.disconnect();
...
async function getAsset(org, id){
    ...
    try {
        ...
        result = await contractOrg.evaluateTransaction('ReadAsset', id);
        console.log('  result: ' + prettyJSONString(result.toString()));
        if (result == null || result == ''){
            console.log('[]');
            gatewayOrg.disconnect();
...
async function createAsset(newAsset, org){
    ...
    try {
        ...
        // values = await returnContractOrg(org);
        // const contractOrg = values[0];
        // gatewayOrg = values[1];
        let exists = await contractOrg.evaluateTransaction('AssetExists', id);
        console.log(`*** Asset Exists: ${prettyJSONString(exists.toString())}`);
        if (exists.toString()==='false'){
            console.log('-> Submit Transaction: CreateAsset ' + id);
            let statefulTxn = contractOrg.createTransaction('CreateAsset');
            //if you need to customize endorsement to specific set of Orgs, use setEndorsingOrganizations
            //statefulTxn.setEndorsingOrganizations(mspOrg1);
...
async function agreeToTransfer(agr, org){
    ...
    try {
        ...
        let statefulTxn = contractOrg.createTransaction('AgreeToTransfer');
...
async function transferAsset(org, bod){
    ...
    try {
        ...
        let statefulTxn = contractOrg.createTransaction('TransferAsset');
...
async function returnContractOrg(org){
    let result = undefined;
    var ccpOrg = undefined;
    var walletOrg = undefined;
    var OrgUserId = undefined;
    ...
    try {
        ...
        await gatewayOrg.connect(ccpOrg,
            { wallet: walletOrg, identity: OrgUserId, discovery: { enabled: true, asLocalhost: true } });
        var networkOrg = await gatewayOrg.getNetwork(myChannel);
        var contractOrg = networkOrg.getContract(myChaincodeName);

    } catch (error) {
        console.error(`Error in connecting to gateway: ${error}`);
        return [null, null];
    } 
    return [contractOrg,gatewayOrg];
}
