...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface) error {
	...
	if len(assetInput.ArtWork) == 0 {
		return fmt.Errorf("artWork field must be a non-empty string")
	}
	if len(assetInput.Artist) == 0 {
		return fmt.Errorf("artist field must be a non-empty string")
	}
	...
	asset := Asset{
		...
		ID:    assetInput.ID,
		ArtWork: assetInput.ArtWork,
		Artist:  assetInput.Artist,
		Owner: clientID,
	}
